...
int main()
{
    ...
    VideoCapture cap(0);  // open the chosen camera feed
    ...
    // Variables //
    ///////////////
    Mat input, diffX, diffY, splinter2[2], splinter3[3];
...
//        imshow("DoG", pyramid[0]);
        ...
        for(int i = 0; i < 7; i++)
        {
            ...
            // Get the optical flow between the previous and current edges
            calcOpticalFlowFarneback(prevPyramid[i], pyramid[i], input, 0.5, 9,
                                    3, 1, 5, 1.2, OPTFLOW_FARNEBACK_GAUSSIAN );
            split(input, splinter2);
            splinter2[0] *= 8;
            splinter2[1] *= 8;
            splinter2[0].convertTo(diffX, CV_8SC1);
            splinter2[1].convertTo(diffY, CV_8SC1);
...
void diff(Mat &src, cv::Mat &dst, bool isXDiff, int gap)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        for (int j = gap; j < cols; j++)
        {
            ...
            for (int k = 0; k < ch; k++)
            {dp[(j-midPixle)*ch+k] = sp[j*ch+k] - sp[(j-gap)*ch+k];}
        }
    }
